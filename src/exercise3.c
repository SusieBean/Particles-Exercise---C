...
int main()
{
    InitWindow(SCREENWIDTH,SCREENHEIGHT,"Particles Exercise 3");
    SetTargetFPS(60);
    ...
    float rate = 0.02f;
    float current = 0.f;
    float movement_rate = 5.f;

    while(!WindowShouldClose())
    {
        system.x += movement_rate;
        if (system.x > SCREENWIDTH || system.x < 0)
        {
            movement_rate *= -1.f;
        }
        ...
        if (current>= rate)
        {
            ...
            if (NewParticle != -1)
            {
                system.particles[NewParticle] = CreateParticle(system.x,system.y);
